...
function create_manufactured_solution(pde, manufactured_solution, u, x, t)
    @info("Creating manufactured solution and forcing term")
    # Define the true solution
    true_sol = build_function(manufactured_solution, [x, t], expression = Val{false})
    
    # Substitute manufactured solution into the provided PDE
    forcing_term = expand_derivatives(substitute(pde, u => manufactured_solution))
...
function modify_script(file_location, outpute_filename, modified_filename, t_final, forcing_term_str, h_value)
    ...
    modified_script = replace(modified_script, "# INSERT_FILE_NAME_HERE" => "\"$outpute_filename\"")
    write(modified_filename, modified_script)
end
...
function compute_l2_error(true_solution, results, h)
    @info("Computing l2 error")
    errs = h*norm(true_solution - results, 2)
